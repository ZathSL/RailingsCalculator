...
class ShowClusters:
    ...
    def crea_segmenti_table(self, frame, cluster_i, i, lista_segmenti):
        tree = ttk.Treeview(frame, columns=(
            "Segmento ID", "Misura ID", "Lunghezza esterna", "Lunghezza interna", "Spessore", "Angolo sinistro",
            "Angolo destro"))

        # Aggiungi colonne
        for col in ["Segmento ID", "Misura ID", "Lunghezza esterna", "Lunghezza interna", "Spessore", "Angolo sinistro",
                    "Angolo destro"]:
            tree.column(col, anchor=tk.W, width=150)
            tree.heading(col, text=col, anchor=tk.W)
        primo_segmento: Segmento = lista_segmenti[0][i].segmento
        data = (0, i, primo_segmento.l_esterna, primo_segmento.l_interna,
                primo_segmento.spessore_sbarra, primo_segmento.angolo_sx,
                primo_segmento.angolo_dx)
        tree.insert("", tk.END, values=data)
        for key, value in cluster_i.items():
            tmp: Misure_segmento = lista_segmenti[key][value.id_misura]
            data = (key, tmp.id_misura, tmp.segmento.l_esterna, tmp.segmento.l_interna,
